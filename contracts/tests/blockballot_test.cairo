...
mod BlockBallotTests {
    ...
    use cairo_contracts::blockballot::{
        BlockBallot, IBlockBallot, IBlockBallotDispatcher, IBlockBallotDispatcherTrait,
    };
    use openzeppelin::token::erc20::{ERC20ABIDispatcher, interface::ERC20ABIDispatcherTrait};
    use debug::PrintTrait;


    fn deploy_contract() -> (ContractAddress, IBlockBallotDispatcher, ContractAddress) {
        ...
        let BlockBallotContract = declare('BlockBallot');
        let contract_address = BlockBallotContract.deploy(@params).unwrap();
        (erc20_token, IBlockBallotDispatcher { contract_address }, contract_address)
    }

    fn create_proposal(asset: ContractAddress, dispatcher: IBlockBallotDispatcher) -> u64 {
